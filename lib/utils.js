...
exports.buildShareUrl = exports.getVtimezoneFromMomentZone = exports.UTCMinsToUTCOffset = exports.escapeICSDescription = exports.isInternetExplorer = exports.isMobile = exports.formatDuration = exports.formatDate = void 0;
...
var padZero = function padZero(n) {
  return "".concat(Math.abs(parseInt(n, 10)) < 10 ? "0" : "").concat(Math.abs(n));
}; // E.g. -600 => +1000


var UTCMinsToUTCOffset = function UTCMinsToUTCOffset(t) {
  // We reverse sign when converting to offset
  var sign = t > 0 ? "-" : "+";
  var hrs = Math.abs(Math.floor(t / 60));
  var mins = Math.abs(t) % 60;
  return "".concat(sign).concat(padZero(hrs)).concat(padZero(mins));
};
/**
...
exports.UTCMinsToUTCOffset = UTCMinsToUTCOffset;
...
var getVtimezoneFromMomentZone = function getVtimezoneFromMomentZone(_ref4) {
  ...
  var footer = "END:VTIMEZONE"; // Find the 'until' index which corresponds with the event start.
  ...
  var currentUntil = zone.untils.findIndex(function (u) {
    return u > (0, _momentTimezone.default)(startDatetime).unix() * 1000;
  }); // Find 'until' index which applies after the event ends.

  var futureUntil = zone.untils.findIndex(function (u) {
    return u > (0, _momentTimezone.default)(endDatetime).unix() * 1000;
  }) + 1;
  var zTZitems = []; // Generate VTIMEZONE entries
  // FIXME: Handle border cases

  for (var i = currentUntil; i < futureUntil + 1; i++) {
    // Determine which mode is starting.
    var observance = (i + 1) % 2 ? "DAYLIGHT" : "STANDARD";
    var tzName = zone.abbrs[i];

    var observanceStartedAt = _momentTimezone.default.tz(zone.untils[i - 1], timezone);

    var offsetFrom = UTCMinsToUTCOffset(zone.offsets[i - 1]);
    var offsetTo = UTCMinsToUTCOffset(zone.offsets[i]);
    zTZitems.push("BEGIN:".concat(observance, "\nDTSTART:").concat(observanceStartedAt.format("YYYYMMDDTHHmmss"), "\nTZOFFSETFROM:").concat(offsetFrom, "\nTZOFFSETTO:").concat(offsetTo, "\nTZNAME:").concat(tzName, "\nEND:").concat(observance));
  }
...
exports.getVtimezoneFromMomentZone = getVtimezoneFromMomentZone;
